...
interface PrimaryButtonProps {
  ...
  onClick?: () => void;
  ctaId?: string;
  extraParams: { source: string } & Record<string, any>;
}
...
export const PrimaryButton = ({
  ...
  useGSAP(
    () => {
      ...
      if (iconRef.current)
        gsap.set(iconRef.current, { x: 0, y: 15, opacity: 0 });
    },
    { scope: btnRef },
  );
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    const { source, ...restParams } = extraParams;
    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-center">
        <div className="relative flex items-center justify-center">
          <span
            ref={textRef}
            className="text-white transition-colors block font-semibold whitespace-nowrap"
          >
            {text}
          </span>
          <div className="absolute left-[97%] w-[1.2rem] h-[1.2rem] overflow-hidden flex items-center justify-center">
            <Icon
              ref={iconRef}
              className="absolute md:w-[1.2rem] w-[1rem] h-[1rem] text-orange"
              strokeWidth={3}
            />
          </div>
        </div>
